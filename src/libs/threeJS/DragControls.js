...
THREE.DragControls = function ( _objects, _camera, _domElement ) {
	...
	function activate() {
		...
		_domElement.addEventListener('dblclick',onDocumentMouseDblClick, false);
	...
	function onDocumentMouseMove( event ) 
	{
		...
		if ( selection && scope.enabled ) {

			if ( _raycaster.ray.intersectPlane( _plane, _intersection ) ) {

				selection.position.copy( _intersection.sub( _offset ) );
				setMeshTransform();
			}

			scope.dispatchEvent( { type: 'drag', object: selection } );
	...
	function onDocumentMouseDown( event ) {
		...
		if ( intersects.length > 0 ) 
		{
			...
			if ( _raycaster.ray.intersectPlane( _plane, _intersection ) &&isDragging) 
			{
				controls.enabled = false;

				_offset.copy( _intersection ).sub( selection.position );
				 setMeshTransform();
			...
			scope.dispatchEvent( { type: 'dragstart', object: selection } );
	...
	function onDocumentMouseCancel( event ) 
	{
		...
		if ( selection ) 
		{
			...
			scope.dispatchEvent( { type: 'dragend', object: selection } );
